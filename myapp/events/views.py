...
from .models import ARTICLE_TYPES
...
def SignupPage(request):
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            subject = 'Airfit : New account creation request'
            ...
            from_email = 'afef.mdhaffar@enis.tn'
            to_email = 'afef.mdhaffar@enis.tn'
...
def approve_user_creation(request, token):
    ...
    if request.method == 'POST':
        if 'approve' in request.POST:
            ...
            subject = 'Aifit : Account creation approved'
            ...
            subject = 'Airfit : Account creation disapproved'
...
def publications(request):
    articles = Article.objects.filter(document__isnull=False)
    context = {}

    for article_type, _ in ARTICLE_TYPES:
        filtered_articles = articles.filter(article_type=article_type)
        print(f"Number of {article_type}: {filtered_articles.count()}")
        context[article_type] = filtered_articles

    return render(request, 'main/publications.html', context)

def generate_bibtex(request, article_id):
    try:
